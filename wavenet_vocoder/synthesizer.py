...
class Synthesizer:
    def load(self, checkpoint_path, hparams, model_name='WaveNet'):
        ...
        self.local_conditions = tf.placeholder(tf.float32, shape=(
            None, None, hparams.num_mels), name='local_condition_features') if local_cond else None
        self.global_conditions = tf.placeholder(tf.int32, shape=(
            None, 1), name='global_condition_features') if global_cond else None
        self.synthesis_length = tf.placeholder(
            tf.int32, shape=(), name='synthesis_length') if not local_cond else None
        self.targets = tf.placeholder(tf.float32, shape=(
            1, None, 1), name='audio_targets') if hparams.wavenet_synth_debug else None  # Debug only with 1 wav
        self.input_lengths = tf.placeholder(tf.int32, shape=(
            1, ), name='input_lengths') if hparams.wavenet_synth_debug else None
    ...
    def synthesize(self, mel_spectrograms, speaker_ids, basenames, out_dir, log_dir):
        ...
        if self.synth_debug:
            ...
            mel_spectrograms = [np.load(mel_file)
                                for mel_file in hparams.wavenet_debug_mels]

        # Get True length of audio to be synthesized: audio_len = mel_len * hop_size
        audio_lengths = [len(x) * get_hop_size(self._hparams)
        ...
        T2_output_range = (-self._hparams.max_abs_value,
                           self._hparams.max_abs_value) if self._hparams.symmetric_mels else (0, self._hparams.max_abs_value)

        if self._hparams.clip_for_wavenet:
            mel_spectrograms = [
                np.clip(x, T2_output_range[0], T2_output_range[1]) for x in mel_spectrograms]

        c_batch = np.stack([_pad_inputs(x, maxlen, _pad=T2_output_range[0])
                            for x in mel_spectrograms]).astype(np.float32)
        ...
        if self.synth_debug:
            ...
            test_wavs = [np.load(debug_wav).reshape(-1, 1)
                         for debug_wav in debug_wavs]

            # pad wavs to same length
            max_test_len = max([len(x) for x in test_wavs])
            test_wavs = np.stack([_pad_inputs(x, max_test_len)
                                  for x in test_wavs]).astype(np.float32)
        ...
        generated_wavs, upsampled_features = self.session.run(
            [self.model.tower_y_hat, self.model.tower_synth_upsampled_local_features], feed_dict=feed_dict)
        ...
        generated_wavs = [generated_wav[:length]
                          for generated_wav, length in zip(generated_wavs, audio_lengths)]
        upsampled_features = [upsampled_feature[:, :length]
                              for upsampled_feature, length in zip(upsampled_features, audio_lengths)]
        ...
        for i, (generated_wav, input_mel, upsampled_feature) in enumerate(zip(generated_wavs, mel_spectrograms, upsampled_features)):
            ...
            if log_dir is not None:
                ...
                util.waveplot(plot_filename, generated_wav, None,
                              hparams, title='WaveNet generated Waveform.')
